...
const STR: Record<string, { zh: string; en: string }> = {
  ...
  'pd.certificate.recordVerified': { zh: '机器核验', en: 'Machine-verified' },
  'pd.certificate.recordWidth': { zh: '相对宽度', en: 'rel. width' },
  'pd.certificate.recordVerify': { zh: '独立复核公证快照', en: 'Re-verify attestation' },
  'pd.certificate.recordAttest': {
    zh: '该括区由参考核验器（contracts/verifier.ts）机器判定，公证快照对任何人可复算。',
    en: 'This bracket is machine-judged by the reference verifier (contracts/verifier.ts); the attestation snapshot is recomputable by anyone.',
  },
  'st.attestLink': { zh: '独立复核机器核验公证快照', en: 'Re-verify machine-verified attestations' },
